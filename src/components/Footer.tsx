...
const SocialLink = ({
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className="group">
      ...
      <Icon
        className={
          "h-5 w-5 transition group-hover:fill-blue-600" +
          " " +
          `${isHome ? "fill-gray-950" : "fill-gray-100"}`
        }
      />
    </a>
  );
...
const Footer = () => {
  ...
  const colorVal = isHome ? "text-gray-800" : "text-gray-200";
  const iconColorVal = isHome ? "text-gray-800" : "text-gray-100";
  return (
    <div className="z-[10] fixed left-0 right-0 bottom-0 backdrop-blur-md bg-violet-400/10 flex flex-row justify-between py-4 px-12 ">
      <p className={"text-xs" + " " + colorVal}>
        Copyright © {new Date().getFullYear()}{" "}
        <Link
          href="mailto:ayushagarwal.dev@gmail.com"
          target="_blank"
          rel="noreferrer"
          className={
            "underline hover:text-blue-500" +
            " " +
            `${isHome ? "text-violet-800" : "text-violet-200"}`
          }
        >
          ayush
        </Link>
